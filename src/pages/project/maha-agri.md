---
templateKey: project-post
title: Maha Agri
domainNew: Recent Programs
id: Maha Agri
heroSection:
  isVisible: true
  title: Maha Agri
  description: Streamlining operations by embedding a culture of data-backed
    decision-making and ensuring 80%+ farmers receive fortnightly advisories
    from the department
  categories:
    - Agriculture
    - Maharashtra
  backgroundImage: /img/mahaagri-picture-1.jpeg
whyImportantSection:
  isVisible: true
  title: Why is it important?
  backgroundImage: /img/mahaagri-picture-2.jpeg
  infoCard:
    title: "1.5 Cr+ farmers in the state of Maharashtra, spread across 33 districts
      and 9 agro-climatic zones\t"
    description: "Even with ~13,000 field officers and an annual budget of over Rs.
      40,000 Cr, the Department of Agriculture struggles to effectively serve
      the state’s diverse farmer base\t"
  items:
    - title: With increasing uncertainties (climatic and economic) in agriculture,
        farmers require support from the state
    - title: Poor internal coordination and processes limit the department’s ability
        to provide this support
    - title: "The problem is compounded by a weak feedback loop: farmers’ voices do
        not reach decision-makers"
programHighlightsSection:
  isVisible: true
  description: ""
  title: Program Highlights
  highlights:
    - description: "Ensuring accountability by increasing data visibility and
        establishing a fixed review cadence\t"
      title: Data-backed Accountability
    - description: Strengthening advisory services by developing a multi-channel
        digital extension system
      title: Digital Agricultural Advisory
    - description: "Embedding farmer-centricity in all departmental operations by
        creating a feedback channel\t"
      title: Farmer Centric Operations
    - description: Simplifying field reporting by bringing all key activities into one
        easy-to-use app
      title: Agri-Extension App
impactSection:
  isVisible: false
  stats:
    isVisible: true
  testimonials:
    isVisible: true
partnersSection:
  isVisible: true
  title: Our Partners
---
